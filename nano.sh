...
if [ $GRID == "SGE" ]; then
  ...
  qsub -A ${ASMPREFIX}_nanopolish -V -pe thread 4 -l mem_free=2g  -hold_jid "${ASMPREFIX}map" -t 1-$NUM_JOBS -cwd -N "${ASMPREFIX}nano" -j y  -o `pwd`/\$TASK_ID.polish.out $SCRIPT_PATH/nanoParallelSGE.sh 0
  ...
  sbatch -J ${ASMPREFIX}_map -D `pwd` --cpus-per-task=8 --mem-per-cpu=2g  --time=72:00:00 -o `pwd`/map.out $SCRIPT_PATH/map.sh > map.submit.out 2>&1
  ...
  for offset in `seq 0 $maxarray $NUM_JOBS`; do 
     ...
     sbatch -J ${ASMPREFIX}_nano -D `pwd` --cpus-per-task=4 --mem-per-cpu=2g  --time=72:00:00 --depend=$job -a 1-$e -o `pwd`/%A_%a.polish.out $SCRIPT_PATH/nanoParallelSGE.sh $offset >> nanoParallel.submit.out 2>&1
  done
  job=`cat nanoParallel.submit.out |awk '{print "afterany:"$NF}' |tr '\n' ',' |awk '{print substr($0, 1, length($0)-1)}'`
  echo "Submitted nanopolish array job $job"
  sbatch -J ${ASMPREFIX}_merge -D `pwd` --cpus-per-task=1 --mem-per-cpu=10g --time=72:00:00 --depend=$job -o `pwd`/merge.out $SCRIPT_PATH/merge.sh
else
